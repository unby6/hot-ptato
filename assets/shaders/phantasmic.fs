...
extern MY_HIGHP_OR_MEDIUMP vec2 phantasmic;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	float t = phantasmic.y*2.221 + time;
    ...
    float res = (.5 + .5* cos( (phantasmic.x) * 2.612 + ( field + -.5 ) *3.14));
    vec4 textp = RGB(hsl);
    tex.rgb = textp.rgb;
    ...
    super_clusters *= super_clusters * super_clusters * 1.6;

	colour.rgb = vec3(0.45, 0.7, 1.) * ((((clusters + stars + super_clusters)+0.1)) * 0.325 + stars*0.05);
