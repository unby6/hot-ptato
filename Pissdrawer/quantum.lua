...
function Card:is_rarity(rarity)
    if self.ability.quantum_1 then
        ...
        local own_rarity_1 = rarities[self.ability.quantum_1.config.center.rarity] or self.ability.quantum_1.config.center.rarity
        local own_rarity_2 = rarities[self.ability.quantum_2.config.center.rarity] or self.ability.quantum_2.config.center.rarity
        return own_rarity_1 == rarity or SMODS.Rarities[own_rarity_1] == rarity or own_rarity_2 == rarity or SMODS.Rarities[own_rarity_2] == rarity
    end
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end, card)
    ...
    if card and card.ability.quantum_1 then
        ...
        local vars = card.ability.quantum_2.config.center.loc_vars and 
            card.ability.quantum_2.config.center.loc_vars(card.ability.quantum_2.config.center, info_queue, card.ability.quantum_2).vars -- custom loc_vars functions
            or Card.generate_UIBox_ability_table({ability = card.ability.quantum_2.ability, config = card.ability.quantum_2.config, bypass_lock = true}, true)
...
function G.UIDEF.card_h_popup(card)
    ...
    if AUT.quantum then
        table.insert(ret_val.nodes[1].nodes[1].nodes[1].nodes, #ret_val.nodes[1].nodes[1].nodes[1].nodes+(card.config.center.discovered and 0 or 1), desc_from_rows(AUT.quantum))
    end
    return ret_val
end
